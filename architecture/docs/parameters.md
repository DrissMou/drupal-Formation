...
---------|---------------|--------------
drupal_project_name | drupaldev | Project name, initialised with composer create-project
...
drupal_cronjob | bash -c 'cd {{ drupal_source_path }} && ../bin/drush cron --uri={{ uri }} >> {{ drupal_cron_log }} 2>&1' | The line that will be added to the crontab
